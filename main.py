...
def highlight_author(
    selected_author: List[str], authors: List[str], color: str = "lightgreen"
) -> str:
    author_list = []
    for a in authors:
...
def get_top_k_authors(papers: List[Paper], k: int = 7) -> List[str]:
    author_list = Counter([a for p in papers for a in p.authors]).most_common(k)
...
def main(
    ...
    st.markdown(
        "[![GitHub](https://img.shields.io/badge/AI_Scholar-GitHub-blue.svg)](https://github.com/chiayewken/aischolar)"
    )
    ...
    with st.sidebar.expander("Select Venue"):
        venues = st.multiselect("Paper Venue", venues, default=venues)
    ...
    vip_authors = get_top_k_authors(papers, top_authors)
    selected_author = st.sidebar.multiselect(
        "Top Authors related to search", vip_authors
    )
    selected_author = [o.split("(")[0] for o in selected_author]
    query = query + " " + " ".join(selected_author)

    page_number = st.sidebar.number_input(
        "Page number", min_value=1, max_value=len(papers) // max_results + 1, value=1
    )
    start = (page_number - 1) * max_results
    end = min(len(papers), page_number * max_results)
    ...
    for p in papers[int(start) : int(end)]:
